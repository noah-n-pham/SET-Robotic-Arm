...
pos_offset = [0, half + 0.002, 0]
aruco_face_id = p.createMultiBody(
    baseMass=0,
    baseVisualShapeIndex=aruco_face,
    basePosition=[0.5 + pos_offset[0], -1.0 + pos_offset[1], 0.6 + pos_offset[2]],
    baseOrientation=p.getQuaternionFromEuler([np.pi/2, -np.pi/2, 0])
)
...
while True:
    ...
    if ee_link_idx is not None:
        ...
        rgb = img_arr[2]
        # Optional: visualize as PIL image
